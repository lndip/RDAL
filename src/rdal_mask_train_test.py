from forward_backward_pass import forward_backward_pass_rdal_mask
...
def train_rdal_mask(batch_size:int, 
                    training:str, 
                    do_grad_clip:bool, 
    ...
    :param P: Period of verification
    :type P: int
    :param patience: Patience for the early stopping
    :type patience: int
    :param ver_patience: Patience for the early stopping in verification
    ...
    # Get the models
    feature_extractor, mask_unet, sound_event_classifier, speech_discriminator, _, _ = get_models(device)

    # Get the data loaders
    train_dataloader, val_dataloader, test_dataloader = get_all_data_loaders(batch_size)

    # Get the optimizer and load pretrained mask if the masked is not optimized
    ...
    # Load the pretrained mask
    _, _, mask_unet = train_masking_network(mask_unet,
    ...
    best_ver_lowest_validation_loss = 0
    best_epoch = 0
    patience_counter = 0
    ...
    epoch_ver_accuracy = []
    epoch_ver_recall = []
    epoch_ver_lowest_validation_loss = []
    ...
    for epoch in range(epochs):
        ...
        # Train the models
        feature_extractor, mask_unet, sound_event_classifier, speech_discriminator, train_loss_sec, train_loss_sd, \
        train_event_accuracy, train_speech_accuracy, train_speech_recall, _, _ = \
            forward_backward_pass_rdal_mask(feature_extractor,
                                            mask_unet,
        ...
        # Validate the models
        with torch.no_grad():
            feature_extractor, mask_unet, sound_event_classifier, speech_discriminator, val_loss_sec, val_loss_sd, \
            val_event_accuracy, val_speech_accuracy, val_speech_recall, _, _ = \
                forward_backward_pass_rdal_mask(feature_extractor,
                                                mask_unet,
                                                sound_event_classifier,
                                                speech_discriminator,
        ...
        if epoch % P == P-1:
            ...
            _, _, _, ver_acc, ver_recall, ver_lowest_validation_loss,_,_ = verification(
                feature_extractor, mask_unet, train_dataloader, val_dataloader, test_dataloader, epochs, ver_patience, device, training, job_idx)
            
            epoch_ver_accuracy.append(ver_acc)
            epoch_ver_recall.append(ver_recall)
            epoch_ver_lowest_validation_loss.append(ver_lowest_validation_loss)
        
            print(f'Verification accuracy - test data: {ver_acc:7.4f} | '
                    f'Verification recall - test data: {ver_recall:7.4f} | '
                    f'Lowest validation loss value: {ver_lowest_validation_loss:7.4f} | ')

            # Early stopping
            if alpha.item() >= 0.99:
                if ver_lowest_validation_loss > best_ver_lowest_validation_loss:
                    best_ver_lowest_validation_loss = ver_lowest_validation_loss
                    best_epoch = epoch
                    patience_counter = 0

                    torch.save(feature_extractor.state_dict(), Path(MODELS_DIR, training, f"{job_idx}", f"best_feature_extractor_{best_epoch}.pt"))
                    torch.save(mask_unet.state_dict(), Path(MODELS_DIR, training, f"{job_idx}", f"best_mask_unet_{best_epoch}.pt"))
                ...
                if patience_counter > patience:
                    print(f'Early stopping criterion met at epoch {best_epoch} | '
                          f'lowest validation loss in verification: {best_ver_lowest_validation_loss:7.4f}')
                    break

            # Load verifying SD to speech discriminator
            if alpha.item() > 0:
                print("Loading verification SAD state dict to adversarial SAD...")
                speech_discriminator.load_state_dict(torch.load(Path(MODELS_DIR, training, f"{job_idx}", "verifying_sd.pt"), map_location=device))
...
def test_rdal_mask(batch_size:int, 
                   training:str, 
                   job_idx:int, 
                   ver_epochs:int, 
                   ver_patience:int, 
    ...
    """Test the RDAL+M model

    :param batch_size: Batch size
    :type batch_size: int
    :param training: The specified training mode
    :type training: str
    :param job_idx: Job index on the cluster
    :type job_idx: int
    :param ver_epochs: Verification epochs
    :type ver_epochs: int
    :param ver_patience: Patience for the early stopping in verification
    :type ver_patience: int
    :param device: The current device
    :type device: str
    :param best_epoch: Best epoch from the training 
    :type best_epoch: int
    ...
    mask_unet.load_state_dict(torch.load(Path(MODELS_DIR, training, f"{job_idx}", f"best_mask_unet_{best_epoch}.pt"), map_location=device))
    ...
    with torch.no_grad():
        _, _, _, _, test_loss_sec, test_loss_sd, test_event_accuracy, test_speech_accuracy, test_speech_recall, test_event_targets, test_event_predictions = \
                forward_backward_pass_rdal_mask(feature_extractor,
                                                mask_unet,
                                                sound_event_classifier,
                                                speech_discriminator,
    ...
    ver_targets, ver_predictions, ver_probs, ver_acc, ver_recall, ver_lowest_validation_loss, ver_acc_val, ver_recall_val = verification(
        feature_extractor, mask_unet, train_dataloader, val_dataloader, test_dataloader, ver_epochs, ver_patience, device, training, job_idx)
