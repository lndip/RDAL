    ...
    for epoch in range(epochs):
        ...
        if epoch % P == P-1:
            ...
            _, _, _, ver_acc, ver_recall, ver_lowest_validation_loss,_,_ = verification(
                feature_extractor, None, train_dataloader, val_dataloader, test_dataloader, epochs, ver_patience, device, training, job_idx)
...
def test_rdal(batch_size:int, 
    ...
    :param job_idx: The job index on the cluster
    :type job_idx: int
    :param ver_epochs: Number of epochs in the verification process
    :type ver_epochs: int
    :param ver_patience: Patience for the early stopping in verification
    :type ver_patience: int
    :param device: The current device
    :type device: str
    :param best_epoch: Best epoch from the training process
    :type best_epoch: int
